...
let arr = [];
let count = 0;
let writeStyles = fs.createwriteHTML(path.join(__dirname, 'project-dist', 'styles.css'));
      ...
      if (stats.isFile()) {
        if (path.extname(files[i]) === '.html') {
          ...
          readHtml.on('data', chunk => {
            ...
            sum++;
            if (htmlCount === sum) {
              writeHTML = fs.createwriteHTML(path.join(__dirname, 'project-dist', 'index.html'));
